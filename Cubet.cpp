#include "cubet.h"
using namespace cubetNS;

CubeT::CubeT() : GenericCube()
{
...
void CubeT::setPieces()
{
	...
	for (auto b : vecPieces)
		b->color = Block::PURPLE;
}

void CubeT::rotate(std::vector<std::vector<std::shared_ptr<Block>>>& map)
{
	int cnk_x = 0;
	int cnk_y = 0;
...
// update each chunck
//=============================================================================
void CubeT::update(float frameTime)
{
	for (auto b : vecPieces)
		b->update(frameTime);
}
