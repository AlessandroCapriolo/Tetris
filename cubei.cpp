#include "cubei.h"
using namespace cubeiNS;

CubeI::CubeI() : GenericCube()
{
...
//=============================================================================
// rotate the cube checking the map
//=============================================================================
void CubeI::rotate(std::vector<std::vector<std::shared_ptr<Block>>>& map)
{
	int cnk_x = 0;
	int cnk_y = 0;
	...
	case GenericCube::SOUTH:
		...
		if (cnk_y == 19 || cnk_y < 2)
			return;
		if (map[cnk_y - 2][cnk_x] != nullptr || map[cnk_y - 1][cnk_x] != nullptr || map[cnk_y + 1][cnk_x] != nullptr)
			return;

		vecPieces[0]->setX(vecPieces[0]->getX() + 2 * blockNS::WIDTH);			
	...
	case GenericCube::WEST:
		...
		if (cnk_x < 2 || cnk_x == 9)
			return;
		if (map[cnk_y][cnk_x - 2] != nullptr || map[cnk_y][cnk_x - 1] != nullptr || map[cnk_y][cnk_x + 1] != nullptr)
			return;
...
// update each chunck
//=============================================================================
void CubeI::update(float frameTime)
{
	for (auto b : vecPieces)
		b->update(frameTime);
}
