#include "cubej.h"
using namespace cubejNS;

CubeJ::CubeJ() : GenericCube()
{
...
void CubeJ::rotate(std::vector<std::vector<std::shared_ptr<Block>>>& map)
{
	int cnk_x = 0;
	int cnk_y = 0;
	...
	case GenericCube::NORTH:
		...
		if (cnk_y == 19 || cnk_y == 0)
			return;
		if (map[cnk_y - 1][cnk_x] != nullptr || map[cnk_y + 1][cnk_x - 1] != nullptr || map[cnk_y + 1][cnk_x] != nullptr)
			return;
	...
	case GenericCube::EAST:
		...
		if (cnk_x == 9)
			return;
		if (map[cnk_y - 1][cnk_x - 1] != nullptr || map[cnk_y][cnk_x - 1] != nullptr || map[cnk_y][cnk_x + 1] != nullptr)
			return;
...
// update each chunck
//=============================================================================
void CubeJ::update(float frameTime)
{
	for (auto b : vecPieces)
		b->update(frameTime);
}
